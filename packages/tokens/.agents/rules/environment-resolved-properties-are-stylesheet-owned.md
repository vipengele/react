# Never add a `--tandiko-*` property to `createTheme`'s output if its value depends on colour mode or `prefers-reduced-motion`

`ThemeProvider` applies `createTheme`'s output as an inline style on `.tandiko-root`. An inline
...
`[data-tandiko-mode="dark"]` rule or the `prefers-reduced-motion` query. The failure is silent:
...
reject it), assign its default in `.tandiko-root` in `base-stylesheet.ts`, and reassign it in the
