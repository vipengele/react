...
corner radius, focus ring, invalid border, disabled dimming, height, horizontal padding and
width. A control that composes it keeps its own element, class name, `className` and prop
...
- The composed control's own stylesheet: no `border`, `background-color`, `border-radius`,
  `box-shadow`, `width`, or state selector for hover/focus-visible/`aria-invalid`/`:disabled` —
