...
The editable lockups in `assets/src/` use **live text** in Poppins (SemiBold wordmark,
Regular tagline). They only render correctly where that font is installed. For distribution you
...
4. `<tspan>` children become separate `<path>` elements, each starting at the `endX` of
   the previous run; a plain `<text>` keeps its `id` (`wordmark`, `tagline`) on the path

A **fitted run** (`textLength` + `lengthAdjust="spacing"`) is solved the same way the
...
# examples
node build-png.js ../dist/tandiko-mark.svg 512
node build-png.js ../dist/tandiko-logo-notagline.svg 800 logo-800.png
node build-png.js ../dist/tandiko-mark-mono-ink.svg 32 favicon-32.png   # ≤16–32px: mono mark
```

If no output path is given, the PNG is written alongside the SVG with a `-<width>` suffix
(e.g. `dist/tandiko-mark-512.png`).
...
```
brand/assets/
  src/      editable masters (text as text — needs Poppins to render correctly)
  dist/     outlined, font-free — use these for embedding / PNG export
  fonts/    Poppins 400/600 TTFs used by the outliner
  tools/
    build-svg.js     reads src/, writes outlined SVGs to dist/
    build-png.js     renders a dist/ SVG to PNG at a given width
```

Dependencies (`opentype.js`, `@xmldom/xmldom`, `@resvg/resvg-js`) are declared in
