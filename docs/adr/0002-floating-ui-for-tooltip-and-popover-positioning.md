...
manage focus and dismissal as a proper overlay. `@tandiko/ui` already takes `@tandiko/icons` as a
...
Decided: `@tandiko/ui` takes `@floating-ui/react` as a real dependency, rather than hand-rolling
...
Both components portal their floating element into the nearest ancestor `.tandiko-root` (found
via `closest(".tandiko-root")` on the trigger), not `document.body`. `ThemeProvider` (ADR 0001)
assigns every `--tandiko-*` custom property on `.tandiko-root`, not `:root` — a portal to
`document.body` would render outside that scope and lose every themed value, silently breaking
dark mode for exactly these two components the same way `--tandiko-surface-dark` and Skeleton's
shimmer already broke it once each. Portaling to the nearest `.tandiko-root` keeps the floating
...
- Portal to `document.body` (floating-ui's default) — rejected: outside `.tandiko-root`'s scope, so every `--tandiko-*` value the floating element reads via `var()` resolves to nothing unless the theme is duplicated onto the portal root, which is the exact problem ADR 0001 avoided a context hook to prevent.
