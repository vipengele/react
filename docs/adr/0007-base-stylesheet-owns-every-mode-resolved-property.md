...
`createTheme`'s output: the rules in `base-stylesheet.ts` select `.vpg-root`, which is the
very element carrying the inline theme.

A `--vpg-*` property belongs to the stylesheet when its own declared value has to differ
between the states of a condition only the cascade knows. Colour mode is one such condition. The
three colours the theme switches wholesale — `--vpg-accent`, `--vpg-ink`,
`--vpg-surface` — turn on it, and so do the two inks every elevation shadow is drawn in,
`--vpg-shadow-contact` and `--vpg-shadow-ambient`, whose alphas have to climb on a dark
ground to register at all. So do the three unitless ramp scalars — `--vpg-state-shift`,
`--vpg-lift`, `--vpg-sink` — read inside `calc()` by the accent and surface ramps, where
`--vpg-state-shift`'s sign is what makes a hover darken on a light ground and lighten on a
dark one.

`prefers-reduced-motion` is a second such condition, and the motion durations
`--vpg-duration-fast`, `--vpg-duration-normal` and `--vpg-duration-slow` turn on it:
under `reduce` they collapse to `0.01ms`. The easings do not — a curve shapes a transition's
progress and says nothing at a collapsed duration — so `--vpg-ease-*` stays in the theme.

Being a colour is not what puts a property in that set, and neither is being a motion token.
Most `--vpg-*` colours are invariant across both conditions: the `-light`/`-dark` variants
hold literal or seed-derived values, and `--vpg-border`, `--vpg-accent-hover`,
`--vpg-surface-raised` and the rest of the ramps are expressions that read a stylesheet-owned
property back through `var()`, so they re-derive themselves the moment a rule reassigns it.

`--vpg-state-shift: -0.05` in `createTheme`'s output is an inline declaration that the
`[data-vpg-mode="dark"]` rule's `--vpg-state-shift: 0.05` cannot override. In dark mode
the hover and press steps darken instead of lightening and the raised/sunken surfaces keep
their light-mode sizes, even though `data-vpg-mode` is set correctly and the colours do
...
**Invariant: a `--vpg-*` property whose value depends on an environment condition the
...
- `.vpg-root` assigns every stylesheet-owned property. The colours resolve through
  `light-dark()` — the seed-derived ones as
  `light-dark(var(--vpg-<x>-light), var(--vpg-<x>-dark))`, the shadow inks as literal
...
- The dark rules reassign only `color-scheme: dark` and the three scalars (`0.05`, `0.055`,
  ...
  declarations regardless. The three dark selectors and their `:not([data-vpg-mode="light"])`
  guards are unaffected.
- The durations take their full-motion values on `.vpg-root` — `120ms`, `200ms`, `320ms` —
  and an `@media (prefers-reduced-motion: reduce)` block matching `.vpg-root` reassigns all
  ...
  `.vpg-root` plainly and touches nothing but the durations, so it composes with whichever
...
- **Derive the scalars from a single sign multiplier**, e.g. `--vpg-mode-sign: 1 | -1` with
  each scalar written `calc(var(--vpg-mode-sign) * <magnitude>)`. Rejected on the values
  themselves: only `--vpg-state-shift` changes sign. `--vpg-lift` widens (`0.02` →
  `0.055`) and `--vpg-sink` narrows (`0.04` → `0.025`), both staying positive, because a
...
- **Encode the mode into one scalar-carrying property** — a 0/1 flag with each scalar written
  `calc(<light> + var(--vpg-mode-flag) * (<dark> - <light>))`. This is arithmetically
...
- **Have `ThemeProvider` pick the dark values in JS and keep applying them inline.** Rejected
  because `colorMode` is optional by design: omitted, no `data-vpg-mode` is written and the
...
`source/react-ui/packages/ui/src/theme.browser.test.ts`. That the durations collapse under
`prefers-reduced-motion: reduce` is covered by `source/react-ui/packages/ui/src/theme-motion.browser.test.ts`,
