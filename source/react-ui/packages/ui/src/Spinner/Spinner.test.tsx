...
describe("Spinner", () => {
  ...
  it("renders at the medium size by default", () => {
    render(<Spinner />);
    expect(screen.getByRole("status")).toHaveClass("vpg-spinner-md");
  });

  it.each(["sm", "md", "lg"] as const)("renders the %s size class", (size) => {
    render(<Spinner size={size} />);
    expect(screen.getByRole("status")).toHaveClass(`vpg-spinner-${size}`);
  });
  ...
  it("composes a caller-supplied className alongside its own classes", () => {
    ...
    expect(spinner).toHaveClass("vpg-spinner");
  });

  it("sets no class attribute beyond its own when no className is given", () => {
    render(<Spinner />);
    expect(screen.getByRole("status").getAttribute("class")).toBe("vpg-spinner vpg-spinner-md");
  });
  ...
  it("never assigns a --vpg-* custom property inline", () => {
    ...
    expect(screen.getByRole("status").getAttribute("style")).not.toContain("--vpg-");
  });
  ...
  it("injects its stylesheet once for any number of spinners", () => {
    ...
    const styles = document.head.querySelectorAll('style[data-href="vpg-spinner"]');
    expect(styles).toHaveLength(1);
    expect(styles[0]?.textContent).toContain("@keyframes vpg-spinner-rotate");
  });

  it("takes its default stroke colour from the accent custom property", () => {
    render(<Spinner />);
    const style = document.head.querySelector('style[data-href="vpg-spinner"]');
